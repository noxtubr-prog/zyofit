...
const sidebarLinks = [
  ...
  { to: "/tailor/store", label: "My Store", icon: Store },
  { to: "/tailor/services", label: "Services", icon: Scissors },
  { to: "/tailor/orders", label: "Orders", icon: Package },
  { to: "/tailor/wallet", label: "Wallet", icon: Wallet },
  { to: "/tailor/withdraw", label: "Withdraw", icon: ArrowDownToLine },
  { to: "/tailor/profile", label: "Profile", icon: User },
];
...
const TailorLayout = () => {
  ...
  return (
    <div className="min-h-screen flex bg-background">
      ...
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-50 w-64 flex flex-col bg-[hsl(156_61%_15%)] transition-transform duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] md:relative md:translate-x-0",
      ...
      >
        <div className="flex h-16 items-center gap-2.5 border-b border-white/10 px-5">
          ...
          <span className="ml-auto text-xs font-medium text-accent bg-white/10 px-2 py-0.5 rounded-md">
            Tailor
          </span>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-primary/20 bg-background/95 backdrop-blur-md px-4 md:px-6">
          ...
          <h1 className="text-lg font-bold text-foreground">
            {sidebarLinks.find(l => isActive(l.to))?.label ?? "Tailor Dashboard"}
          </h1>
        </header>
